...
#'    Required columns are YEAR, WEEK, and \% WEIGHTED ILI
...
getIncidenceFromILI <- function(year, ILI = cdcfluview::get_flu_data(region = "national", years = (year-1):(year+1)),
  ...
  data.year <- ILI %>% dplyr::filter(( YEAR == year & WEEK >= startWeekOfSeason) | (YEAR == (year + 1) & WEEK < startWeekOfSeason)) %>%
    dplyr::select(WEEK, `% WEIGHTED ILI`) %>% dplyr::rename(ILI = `% WEIGHTED ILI`) %>% dplyr::filter(!is.na(ILI))
